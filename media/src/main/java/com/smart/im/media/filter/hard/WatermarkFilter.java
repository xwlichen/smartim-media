...
import com.smart.im.media.utils.opengl.ShaderHelper;
import com.smart.im.media.utils.opengl.TextureHelper;
...
public class WatermarkFilter extends BaseHardVideoFilter {
    ...
    protected String fragmentshader_filter = "" +
            ...
            "   }\n" +
            "}";
    protected int imageTexture = OpenGLConstants.NO_TEXTURE;
    ...
    public void onInit(int VWidth, int VHeight) {
        ...
        glProgram = ShaderHelper.buildProgram(vertexShader_filter, fragmentshader_filter);
    ...
    public void onDraw(int cameraTexture, int targetFrameBuffer, FloatBuffer shapeBuffer, FloatBuffer textrueBuffer) {
        synchronized (syncBitmap) {
            if (needUpdate) {
                if (imageTexture != OpenGLConstants.NO_TEXTURE) {
                    GLES20.glDeleteTextures(1, new int[]{imageTexture}, 0);
                }
                imageTexture = TextureHelper.loadTextureByBitmap(iconBitmap, OpenGLConstants.NO_TEXTURE);
        ...
        LogUtils.e("SIZE_HEIGHT:" + SIZE_HEIGHT + "SIZE_WIDTH:" + SIZE_WIDTH);
        LogUtils.e("iconRectF.top:" + iconRectF.top + ",iconRectF.bottom:" + iconRectF.bottom + ",iconRectF.left:" + iconRectF.left + ",iconRect.right:" + iconRect.right);
