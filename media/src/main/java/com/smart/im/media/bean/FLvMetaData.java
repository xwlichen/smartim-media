...
public class FLvMetaData {
    ...
    public FLvMetaData(PushConfig config) {
        this();
        //Audio
        //AAC
        setProperty("audiocodecid", 10);
        switch (config.getAudioBitRate()) {
            case 32 * 1024:
        ...
        switch (config.getAudioSampleRate()) {
            case 44100:
        ...
        setProperty("framerate", config.getFps().value());
        setProperty("width", config.getWidth());
        setProperty("height", config.getHeight());
    }
